...
export async function cmdInit(projectRoot: string, overwrite: boolean): Promise<void> {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const extracted = repository ? extractGithubRepo(repository) : null;

    if (!extracted) {
      // Still include it, but without a repo/subpath. 
      // It will use npm_tarball source if available.
      resolved.set(name, { repo: "", description: info?.description ?? undefined });
    } else {
  ...
  console.log(chalk.green(`Resolved ${resolved.size} packages (using npm_tarball where repo is missing).`));

  let toml = "";
  toml += "# Generated by `ai-fdocs init`\n";
  toml += "[settings]\n";
  toml += 'output_dir = "fdocs/node"\n';
  toml += "prune = true\n";
  ...
  for (const [name, info] of resolved) {
    ...
    toml += `[packages.${key}]\n`;
    if (info.repo) toml += `repo = "${info.repo}"\n`;
